...
func (s *servidor) AttackDiaboromon(ctx context.Context, req *pb.AttackRequest) (*pb.AttackResponse, error) {
	...
	if req.AttackValue >= s.CD {
		fmt.Println("Diaboromon ha sido derrotado.")
		s.derrota <- struct{}{}
		s.cancelar()
...
func main() {
	...
	// Inicia el servidor gRPC de Diaboromon
	go func() {
	...
	// Espera la señal de inicio de Tai
	<-s.inicio

	// Comienza a atacar a Tai cada TD segundos
	go func() {
	...
	// Espera derrota o victoria
	select {
	case <-s.derrota:
		// Diaboromon ha sido derrotado
	case <-s.victoria:
		// Diaboromon ha ganado
	}
	// Da tiempo para que las gorutinas terminen
