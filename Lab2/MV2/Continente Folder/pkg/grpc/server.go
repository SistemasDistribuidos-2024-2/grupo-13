...
func (s *RegionalServer) TerminateRegional(ctx context.Context, req *pb.TerminateRequest) (*pb.TerminateResponse, error) {
	log.Printf("[CONTINENTE FOLDER] Señal de terminación recibida: %s", req.Message)
...
func StartServer(address string, server *RegionalServer) error {
	...
	if err != nil {
		log.Fatalf("[ERROR][CONTINENTE FOLDER] Error al iniciar el servidor: %v", err)
		return err
	}
	...
	log.Printf("[CONTINENTE FOLDER] Servidor escuchando en %s", address)
