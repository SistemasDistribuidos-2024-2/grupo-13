...
func (c *Client) SendEncryptedMessage(digimon models.Digimon) error {
	...
	log.Printf("[ISLA FILE] Enviado (encriptado): %s", encryptedMessage)
	return nil
}
...
func (c *Client) SendRandomData(count int) {
	for i := 0; i < count; i++ {
		if len(c.digimons) == 0 {
			log.Println("[ISLA FILE] No quedan más Digimon para enviar.")
			return
		}
		...
		if err := c.SendEncryptedMessage(c.digimons[index]); err != nil {
			log.Printf("[ERROR][ISLA FILE] Error al enviar mensaje encriptado: %v", err)
		}
