...
func main() {
	...
	if *peerPorts != "" {
		for _, peerPort := range strings.Split(*peerPorts, ",") {
			if peerPort != *port { // No conectarse a sí mismo
				...
				if err == nil {
					...
					fmt.Printf("[Servidor Hextech] No se pudo conectar al peer en el puerto [%s]: %v\n", peerPort, err)
				}
			}
		}
	}
